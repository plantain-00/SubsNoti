...
export async function update(request: libs.Request, response: libs.Response) {
    ...
    try {
        ...
        if (avatarFileName) {
            ...
            let json = await services.request.postAsync(`http://${settings.config.imageUploader.outerHostName}:${settings.config.imageUploader.port}/api/images/persistent?v=0.12.3`, {
                name: avatarFileName,
                newName: newName,
            });
