# Summary

* [Introduction](README.md)
* [Create an authentication credential for a given email](Create an authentication credential for a given email.md)
* [Get authentication credential](Get authentication credential.md)
* [Delete authentication credential](Delete authentication credential.md)
* [Get current user](Get current user.md)
* [Create an organization](Create an organization.md)
* [Get organizations](Get organizations.md)
* [Create a theme](Create a theme.md)
* [Get themes](Get themes.md)
* [Database models](Database models.md)
* [Error codes](Error codes.md);