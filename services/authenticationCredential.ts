...
export async function authenticateCookie(cookie: string): Promise<libs.ObjectId> {
    ...
    const reply = await services.cache.client.get(settings.cacheKeys.user + authenticationCredential);
    if (reply) {
    ...
    if (libs.md5(user.salt + milliseconds + userId) === tmp[0]) {
        services.cache.client.set(settings.cacheKeys.user + authenticationCredential, userId, "EX", 8 * 60 * 60);
